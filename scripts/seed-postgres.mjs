...
log(`Reading ${join(IN_DIR, 'findings.json')}...`);
const findings = JSON.parse(readFileSync(join(IN_DIR, 'findings.json'), 'utf-8'));
log(`Loaded ${findings.length} findings`);

const lines = [];
lines.push('-- Generated by scripts/seed-postgres.mjs');
lines.push(`-- ${findings.length} findings from aegis-seed pipeline`);
...
if (TRUNCATE) {
  lines.push('-- Clear existing data (cascade to compliance_mappings)');
  lines.push('TRUNCATE findings CASCADE;');
  lines.push('');
}

// ── Compliance frameworks ────────────────────────────────────────────────────

lines.push('-- Ensure compliance frameworks exist');
const frameworks = [
...
for (const [id, name, desc, ver, cat, controls] of frameworks) {
  lines.push(`INSERT INTO compliance_frameworks (id, name, description, version, category, total_controls, controls_passing, controls_failing, score)`);
  lines.push(`  VALUES (${esc(id)}, ${esc(name)}, ${esc(desc)}, ${esc(ver)}, ${esc(cat)}, ${controls}, ${Math.round(controls * 0.72)}, ${Math.round(controls * 0.28)}, ${(72 + Math.random() * 8).toFixed(2)})`);
  lines.push(`  ON CONFLICT (id) DO UPDATE SET total_controls = EXCLUDED.total_controls, score = EXCLUDED.score;`);
}
lines.push('');
...
lines.push(`-- ${findings.length} findings in batches of ${BATCH_SIZE}`);

for (let i = 0; i < findings.length; i += BATCH_SIZE) {
  const batch = findings.slice(i, i + BATCH_SIZE);
  lines.push(`INSERT INTO findings (${COLS.join(', ')}) VALUES`);
...
// ── Compliance mappings (junction table) ─────────────────────────────────────

lines.push('-- Compliance mappings (junction table)');
let mappingCount = 0;
...
for (let i = 0; i < findings.length; i += BATCH_SIZE) {
  ...
  if (mappingRows.length > 0) {
    lines.push('INSERT INTO compliance_mappings (finding_id, framework_id, control_id, control_title, section, severity) VALUES');
    lines.push(mappingRows.join(',\n'));
    lines.push('ON CONFLICT (finding_id, framework_id, control_id) DO NOTHING;');
    lines.push('');
  }
}
...
lines.push('COMMIT;');
lines.push('');
lines.push(`-- Summary: ${findings.length} findings, ${mappingCount} compliance mappings, ${frameworks.length} frameworks`);
...
log(`  ${findings.length} findings, ${mappingCount} compliance mappings, ${frameworks.length} frameworks`);
