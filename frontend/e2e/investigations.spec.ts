...
test.describe('Investigations page', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Wait for the investigation board heading
    await page.getByText('Investigation Board').waitFor({ timeout: 15_000 })
  })

  test('finding list renders', async ({ page }) => {
    // The left panel shows finding items as buttons with severity badges
    const findingItems = page.locator('button').filter({ hasText: /CRITICAL|HIGH|MEDIUM|LOW/ })
    const count = await findingItems.count()
    expect(count).toBeGreaterThan(0)
  })

  test('clicking different findings changes graph center node', async ({ page }) => {
    // Get all finding buttons in the left panel
    const findingButtons = page.locator('.w-80 button').filter({ hasText: /CRITICAL|HIGH|MEDIUM|LOW/ })
