...
const SAMPLE_FINDING: Finding = {
  ...
  description: 'An exposed workload has a reachable path into a sensitive database tier.',
  ...
  ai_risk_score: 8.7,
  ai_risk_level: 'high',
  ai_risk_rationale: 'Exposure plus privilege chaining makes this materially exploitable.',
...
const SAMPLE_PATH: AttackPath = {
  ...
  description: 'Internet exposure pivots through over-permissive access to a database.',
  severity: 'HIGH',
  score: 88,
  hop_count: 3,
  entry_point: {
...
describe('Finding investigation workspaces', () => {
  it('renders the attack-path load state inline under the finding', () => {
    ...
    expect(screen.getByText('Load attack-path analysis for this finding')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /load attack path analysis/i })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /open security graph context/i })).toBeInTheDocument()
  })
  ...
  it('renders security-graph timeline and control context inline', () => {
    renderWithProviders(
      <FindingSecurityGraphWorkspace
        ...
        ticketLinked
        enrichment={{
          root_cause: 'Exposure and excessive identity trust create a direct path to data.',
          impact: 'An attacker can move from the public workload into the database plane.',
    ...
    expect(screen.getByText('internet_exposed && data_access')).toBeInTheDocument()
  })
})
