...
const KPI_CARDS = [
  { label: 'Active Policies', value: 42, sub: '3 pending review', icon: FileText, color: 'text-blue-600', bg: 'bg-blue-50' },
  { label: 'AI Agents', value: 7, sub: '5 active, 2 idle', icon: Bot, color: 'text-indigo-600', bg: 'bg-indigo-50' },
  { label: 'Compliance Score', value: '84%', sub: '+2% this week', icon: ShieldCheck, color: 'text-green-600', bg: 'bg-green-50' },
  { label: 'Open Exceptions', value: 12, sub: '3 critical SLA', icon: AlertTriangle, color: 'text-orange-600', bg: 'bg-orange-50' },
]
        ...
        {TREND.map(({ label, value, change }) => (
          <Card key={label}>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <span className={`ml-auto text-xs font-medium ${change.startsWith('+') ? 'text-green-600' : 'text-red-600'}`}>{change}</span>
            </CardContent>
          </Card>
        ))}
      </div>
            ...
            <TableBody>
              {EXCEPTION_QUEUE.map(exc => (
                <TableRow key={exc.id}>
                  ...
                  <TableCell>
                    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${STATUS_COLORS[exc.status] ?? 'bg-gray-100 text-gray-700'}`}>
                      {exc.status}
                    </span>
                  </TableCell>
                  <TableCell className={`text-xs font-medium ${exc.sla === '—' ? 'text-muted-foreground' : 'text-orange-600'}`}>{exc.sla}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
